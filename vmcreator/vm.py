...
def create_team(output_path, game_hash, team_id, root_key, team_key,
    ...
    try:
        run_cmd(['sudo', 'mount', '--bind', '/dev', '{}/dev'.format(mntdir)],
                "dev bind")
...
def create_org(output_path, game_hash, game_name, teams, services,
               root_keyfile, remote):
    # XXX: see also create_team
    vmdir = clone_vm(output_path, game_hash, "Organization-base",
                     "Organization", remote)
    ...
    try:
        ...
        status(game_hash,
               "Configuring the organization DB, dashboard, bots...",
               remote)
        ...
        for service_name in services:
            ...
            with open("info.json") as jsonfile:
                ...
                info['benign'] = [read_service_script(b)
                                  for b in info['benign']]
                infos.append(info)
        combined_info_json = json.dumps(infos, ensure_ascii=False, indent=1)
        mountdir_writefile(mntdir, "/opt/database/combined_info.json",
                           combined_info_json)
...
""".format(game_name)
        ...
        team_ips = []
        for team_id, team in enumerate(teams, start=1):
            team_ips.append({'team_id': team_id, 'ip': '10.7.{}.2'
                             .format(team_id)})
        scorebot_config = json.dumps(team_ips, indent=1)
        mountdir_writefile(mntdir, '/opt/scorebot/team_list.json',
                           scorebot_config)
...
echo
echo
echo "Done with the first setup! Check that everything is working and start \
your CTF! (run 'start gamebot')"
