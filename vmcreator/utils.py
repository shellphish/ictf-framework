...
class ArgParser(object):
    def parse(self, argv):
        ...
        parser.add_argument('--log', dest='log_path', action='store', default='/tmp')
        ...
        if args.game_hash:
            ...
            game = requests.get("http://ictf.cs.ucsb.edu/framework/ctf/{}".format(args.game_hash)).json()
            args.game = game
            ...
            parser.error('Please specify either a hash (remote) or a json descriptor (local)')
        return args
...
def bundle(game_hash, vm_name, key_name, team_hash, output_dir, remote):
    ...
    logging.info("Putting the {} bundle in {}".format(vm_name, bundle_filename))
